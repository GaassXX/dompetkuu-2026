...
<body class="min-h-screen flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-md">
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 p-8">
            ...
            {{-- Alert success --}}
            @if(session('status'))
                <div class="bg-emerald-50 border border-emerald-200 text-emerald-600 text-xs rounded-xl px-3.5 py-3 mb-5 flex items-center gap-2">
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="mb-5">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}"
                        ...
                        class="w-full px-4 py-3.5 bg-white border border-gray-200 rounded-xl text-sm placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 transition {{ $errors->has('email') ? 'border-red-400' : '' }}"
                        required autofocus>
                </div>
            ...
            <div class="mt-6 text-center">
                <a href="{{ route('login') }}" class="text-sm text-emerald-600 font-semibold hover:underline inline-flex items-center gap-1">
